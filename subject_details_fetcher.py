...
def fetch_subject_details():
    ...
    try:
        ...
        for event in root.findall('.//event'):
            ...
            module = resources.find('module').find('item').text if resources and resources.find('module') else 'N/A'
            room = resources.find('room').find('item').text if resources and resources.find('room') else 'N/A'
            staff = resources.find('staff').find('item').text if resources and resources.find('staff') else 'N/A'
            section = resources.find('group').find('item').text if resources and resources.find('group') else '1'
        ...
        print(f"(PASS) Found a total of {count} timetable slots. Converting to list of dictionaries...")

        # Convert timetable slots to a list of dictionaries
        timetable_slots_dict = [slot.to_dict() for slot in timetable_slots]

        print("(PASS) JSON created successfully")

        # Return the JSON
        return jsonify(timetable_slots_dict)
    except requests.exceptions.Timeout:
        print(f"(FAIL) The request timed out")
